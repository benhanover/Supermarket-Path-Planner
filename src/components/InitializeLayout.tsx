...
const InitializeLayout: React.FC = () => {
  const { setSupermarket, setSupermarketId } = useAppContext();
  const [formData, setFormData] = useState({
    supermarketName: "",
    address: "",
    layoutRows: 20,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get the new supermarket ID
      const supermarketId = newSupermarket.id;

      // Update the supermarket ID in context
      setSupermarketId(supermarketId);

      // Update the supermarket state in context
      setSupermarket({
        name: formData.supermarketName,
      ...
      console.log("Supermarket created successfully:", supermarketId);
    } catch (error) {
      console.error("Failed to initialize supermarket:", error);
      setError("Failed to initialize supermarket. Please try again.");
    } finally {
