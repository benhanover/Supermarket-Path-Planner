...
const ProductSquareEditor = () => {
  ...
  const { selectedSquare, setActiveTab, setActiveAction } = useDashboard();
  ...
  return (
    <div className="flex flex-col items-center gap-2 md:gap-4 p-2 md:p-6 overflow-auto">
      ...
      {/* ✅ ProductsEditor בלי DisplaySquare בפנים */}
      {showProductsEditor && (
        <ProductsEditor mode="square" />
      )}
    </div>
  );
};
